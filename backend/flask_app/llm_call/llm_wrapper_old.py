...
# https://poe.com
# Please store these values in .env file
tokens = { 
    'p-b':          os.getenv('P-B'),               # cookie
    'p-lat':        os.getenv('P-LAT'),             # cookie (poe-tchannel-channel)
    'formkey':      os.getenv('POE_FORMKEY'),       # generated when launch, but needed for passing challenge sometimes
    'cf_clearance': os.getenv('POE_CF_CLEARANCE')   # cookie
}
...
async def llm_response(message):
    ...
    if chat_code:
        client.purge_conversation(bot, chatCode=chat_code)
    return response
