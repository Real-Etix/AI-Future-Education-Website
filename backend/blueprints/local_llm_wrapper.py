...
async def local_llm_response(prompt, temperature=0, max_tokens=1024):
    '''
    ...
    response = classify_llm.create_completion(prompt, **generation_kwargs)
...
意圖：'''
        print(test_random_message[i], '->', asyncio.run(local_llm_response(prompt1, max_tokens=5)))